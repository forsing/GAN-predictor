...
def load_and_normalize_data(csv_file=_DEFAULT_CSV):

    df = pd.read_csv(csv_file, encoding="utf-8")

    if "Powerball" in df.columns and "num7" not in df.columns:
        df = df.rename(columns={"Powerball": "num7"})
...
def train_gan_and_save(
    ...
    model_dir=_V2_MODEL_DIR,
    generator_file="G_v2.pth",
    discriminator_file="D_v2.pth",
    num_epochs=20000,
    batch_size=32,
    z_dim=16,
    log_every=None,
